                    ...
                    <form action="{!! route('buktiTransferPost') !!}" id="form-submit" method="post">
                        ...
                        <input type="hidden" name="no_bukti" id="no_bukti_tf">
                            <div class="card-body"> 
                                <div class="row">
                                    <div class="col-lg-12">
                                        <table class="table table-bordered table-hover" id="tableKomponenKontrak" width="100%">
                                            <thead>
                                                <tr>
      ...
      <div class="modal-dialog modal-xl">
        <div class="modal-content" style="width: 120%">
          ...
          <div class="modal-body">
            ...
            <table class="table table-bordered table-hover" id="tableBuktiTranfer" width="100%">
                <thead>
                    <tr>
                        <th class="text-center text-uppercase">#</th>
    ...
    <div class="modal fade" id="komponen-biaya-modal" aria-hidden="true" style="display: none;">
      <div class="modal-dialog modal-xl">
        <div class="modal-content" style="width: 120%">
          ...
          <div class="modal-body">
            <table class="table table-bordered table-hover" id="tableKomponenBiaya" width="100%">
                <thead>
                    <tr>
                        <th class="text-center text-uppercase">#</th>
        ...
        $(()=>{ 
            let tableKomponenKontrak = $('#tableKomponenKontrak').DataTable({
                initComplete: function(settings, json){
                     $('#tableKomponenKontrak').wrap("<div style='overflow:auto; width:100%;position:relative;'></div>")
                }, 
                processing: true,    
                paging: false,
                searching: false,
            ...
            let tableBuktiTransfer = $('#tableBuktiTranfer').DataTable({
                initComplete: function(settings, json){
                     $('#tableBuktiTranfer').wrap("<div style='overflow:auto; width:100%;position:relative;'></div>")
                }, 
                bSort: false, 
                processing: true,  
                ajax: "{!! route('firmAPI') !!}",
            ...
            let tableKomponenBiaya = $('#tableKomponenBiaya').DataTable({
                initComplete: function(settings, json){
                     $('#tableKomponenBiaya').wrap("<div style='overflow:auto; width:100%;position:relative;'></div>")
                }, 
                bSort: false,
                processing: true,   
                bDestroy: true,  
                ...
                $.ajax({
                    ...
                    success: function(res){ 
                        ...
                        tableKomponenKontrak.row.add([
                            ...
                            `${res.nominalParse} <input type='hidden' class='nominalMurni-${count}' value='${res.nominal}'>`,
                            `<input type='hidden' name='item_kontrak[]' value='${res.id_kontrak}'><input type='text' data-name="${count}" name='alokasi_dana[]' class='form-control item-dana' value='${format(res.nominal)}'>`,
                        ...
                        $('#finalDana').val(format(uangFinal))                         
            ...
            $('#tableKomponenKontrak tbody').on('keyup','.item-dana',function(e){
                ...
                if(val != ''){
                    ...
                    $(".item-dana").each(function(){
                        ...
                        var num = parseFloat(val);
                        total = total + num; 
                        $("#total-dana").val(format(parseFloat(total)));
                    });
                }else{
                    $(this).val(0)
                }
            ...
            $('#tableKomponenKontrak tbody').on('change','.item-dana',function(){
                ...
                $(".item-dana").each(function(){
                    ...
                    murni = $('.nominalMurni-'+id).val()
                    console.log(`get Num : ${num}`)
                    console.log(`get Murni : ${murni}`)
                    console.log(num > parseFloat(murni))
                    if(num > parseFloat(murni)){
                        alert('Alokasi dana melebihi nilai kontrak')
                        $(this).val(0)
                        let finalDana = $('#finalDana').val();
                        $("#total-dana").val(format(parseFloat(finalDana))); 
                    }else{
                        $('#finalDana').val(total)
                        $("#total-dana").val(format(parseFloat(total)));
                    }
